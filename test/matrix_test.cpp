...
{
	int i = 0;
	int dim = round(sqrt(m.get_array().size()));
	for (double x : m.get_array())
	{	
		if (i % dim == 0) cout << "\n";
		cout << x << " ";
		i++;
...
TEST_CASE("can represent 2x2 matrix")
{
	Matrix m({
		-3, 5,
		1, -2 });
...
TEST_CASE("can represent 3x3 matrix")
{
	Matrix m({
		-3, 5, 0,
		1, -2, -7,
...
TEST_CASE("identical matrices are equal")
{
	Matrix a({
		1, 2, 3, 4,
		5, 6, 7, 8,
...
TEST_CASE("different matrices are not equal")
{
	Matrix a({
		1, 2, 3, 4,
		5, 6, 7, 8,
...
TEST_CASE("multiplying 2 matrices")
{
	Matrix a({
		1, 2, 3, 4,
		5, 6, 7, 8,
...
TEST_CASE("mutliplying matrix by tuple")
{
	Matrix a({
		1, 2, 3, 4,
		2, 4, 4, 2,
...
TEST_CASE("multiplying matrix by id matrix returns original matrix")
{
	Matrix a({
		0, 1, 2, 4,
		1, 2, 4, 8,
		2, 4, 8, 16,
		4, 8, 16, 32 });
	Matrix idm = Matrix::id(4);
	REQUIRE(a * idm == a);
}

TEST_CASE("multiplying id matrix by tuple returns same tuple")
{
	Matrix idm = Matrix::id(4);
	tuple a{ 1, 2, 3, 4 };
	REQUIRE(idm * a == a);
...
TEST_CASE("transposing a matrix swaps rows with columns")
{
	Matrix a({
		0, 9, 3, 0,
		9, 8, 0, 8,
...
TEST_CASE("transposing id matrix returns id matrix")
{
...
TEST_CASE("is_invertible returns true for invertible matrix")
{
	Matrix a({
		6, 4, 4, 4,
		5, 5, 7, 6,
...
TEST_CASE("is_invertible returns false for non invertible matrix")
{
	Matrix a({
		-4, 2, -2, -3,
		9, 6, 2, 6,
...
TEST_CASE("calculating the inverse of a matrix")
{
	...
	REQUIRE(b.at(3, 2) == (double) -160 / 532);
	REQUIRE(a.cofactor(3, 2) == 105);
	REQUIRE(b.at(2, 3) == (double) 105 / 532);
	Matrix inverted({
...
TEST_CASE("inverse of second matrix")
{
	// extra tests to ensure method working
	Matrix a({
		8, -5, 9, 2,
		7, 5, 6, 1,
		-6, 0, 9, 6,
	...
	REQUIRE(a.inverse() == inverted);
}

TEST_CASE("inverse of third matrix")
{
	Matrix a({
		9, 3, 0, 9,
		-5, -2, -6, -3,
